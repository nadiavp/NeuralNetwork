...
def run_horizon(timestep):
    ...
    def electric_interrelation(index):
        ...
        return x_m[m,t]>=y_mn[i_lines[RANGE],t]**2 + z_mn[i_lines[RANGE],t]**2

    def D1_def(index):
        t, m = index
        #n = [i for i in range(n_nodes) if network[i].name in network[m].electrical.connections]
        i_lines = e_lines_by_node[m]
        return D1_mn[i_lines[RANGE],t] == 2*y_mn[i_lines[RANGE],t]

    def D2_def(index):
        t, m = index
        i_lines = e_lines_by_node[m]
        return D2_mn[i_lines[RANGE],t] == 2*z_mn[i_lines[RANGE],t]

    def D3_def(index):
        t, m = index
        n = [i for i in range(n_nodes) if network[i].name in network[m].electrical.connections]
        i_lines = e_lines_by_node[m]
        return D3_mn[i_lines[RANGE],t] == x_m[m,t] - x_m[n[RANGE],t]
    
    def D4_def(index):
        t, m = index
        n = [i for i in range(n_nodes) if network[i].name in network[m].electrical.connections]
        i_lines = e_lines_by_node[m]
        return D4_mn[i_lines[RANGE],t] == x_m[m,t] -x_m[n[RANGE],t]

    def D_relations(index):
        t, m = index
        i_lines = e_lines_by_node[m]
        return D1_mn[i_lines[RANGE],t]**2 + D2_mn[i_lines[RANGE],t]**2 + D3_mn[i_lines[RANGE],t]**2 <= D4_mn[i_lines[RANGE],t]**2
    ...
    D1_mn = VariableGroup("D1_mn", indexes = index_e_lines)
    D2_mn = VariableGroup("D2_mn", indexes = index_e_lines)
    D3_mn = VariableGroup("D3_mn", indexes = index_e_lines)
    D4_mn = VariableGroup("D4_mn", indexes = index_e_lines)
    #heat network
    ...
    #lines_index = (range(n_e_lines),)
    add_constraint("D1_def", index_hour + m_index, D1_def)
    add_constraint("D2_def", index_hour + m_index, D2_def)
    add_constraint("D3_def", index_hour + m_index, D3_def)
    add_constraint("D4_def", index_hour + m_index, D4_def)
    add_constraint("D_relations", index_hour + m_index, D_relations)
    ...
    tic = time.time()
    result = prob.solve(verbose = True, solver='MOSEK', NumericFocus=3, gp=True)#solver = 'ECOS_BB')#
