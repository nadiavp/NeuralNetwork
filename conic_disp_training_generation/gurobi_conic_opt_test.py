...
for i in range(len(gen)):
    if isinstance(gen[i], ElectricChiller):
        ...
        fit_terms, x_min, x_max = piecewise_quadratic(gen[i].output.capacity, gen[i].output.heat, max_cap=gen[i].size)
        ...
        fit_terms, x_min, x_max = piecewise_quadratic(gen[i].output.capacity, gen[i].output.electricity, max_cap=gen[i].size)
        setattr(gen[i], 'fundata', {"fp": fit_terms[1], "hp": fit_terms[2], "cp": fit_terms[0], "fq": 0.2*fit_terms[1], "hq": 0.2*fit_terms[2], "cq": 0.5*fit_terms[0]})
        setattr(gen[i], 'ub', x_max)
        setattr(gen[i], 'lb', x_min)
...
# heat_loss[2, 5] = 1#0.91
# heat_loss[2, 6] = 1#0.9
# heat_loss[4, 6] = -1
# heat_loss[4, 7] = 1#0.9
# heat_loss[5, 2] = -1
# heat_loss[5, 6] = 1#0.98
# heat_loss[6, 2] = -1
# heat_loss[6, 4] = 1#0.9
# heat_loss[6, 5] = -1
# heat_loss[7, 4] = -1 
...
n_nodes = 1#len(network)
states = []
constraints = []
date_range = [start_date+datetime.timedelta(hours=i) for i in range(T)]
## create list of components of each type at each node
grid_by_node = [[0]]#find_nodes(grid_para)
turbine_by_node = [list(range(n_turbines))]#find_nodes(turbine_para)
diesel_by_node = [list(range(n_dieselgen))]#find_nodes(diesel_para)
boiler_by_node = [list(range(n_boilers))]#find_nodes(boiler_para)
chiller_by_node = [list(range(n_chillers))]#find_nodes(chiller_para)
abs_by_node = [list(range(n_abs))]#find_nodes(abs_para)
e_storage_by_node = [list(range(n_e_storage))]#find_nodes(e_storage_para)
h_storage_by_node = [list(range(n_h_storage))]#find_nodes(h_storage_para)
c_storage_by_node = [list(range(n_c_storage))]#find_nodes(c_storage_para)
renew_by_node = [[0]]#find_nodes(renew_para)
...
def electric_p_balance(index):
    ...
    return (cvxpy.sum([turbine_xp[j,t] for j in i_turb])\
     + ep_elecfromgrid[m,t] - ep_electogrid[m,t]\
...
def electric_q_balance(index):
    ...
    return (cvxpy.sum([turbine_xq[j,t] for j in i_turb])\
     ...
     cvxpy.sum([z_mn[i_lines[j],t] for j in range(len(n))])
...
def heat_balance(index):
    ...
    return cvxpy.sum([boiler_x[j,t] for j in i_boiler])\
     ...
     - heat_dump[m,t]\
     + heat_unserve[m,t]\
     == 0 #- cvxpy.sum([heat_loss[m,n[j]]*h_mn[i_lines[j],t] for j in range(len(n))])\
...
def cool_balance(index):
    ...
    return cvxpy.sum([abs_x[j,t] for j in i_abs]) + cvxpy.sum([chiller_x[j,t] for j in i_chiller])\
     ...
     0#cvxpy.sum([cool_loss[m,n[j]]*c_mn[i_lines[j],t] for j in range(len(n))])
...
# def line_electric_p(index):
#     t, m = index
#     m_line = e_lines_by_node[m]#index of line from m to n
#     n = [i for i in range(n_nodes) if network[i].name in network[m].electrical.connections]
#     n_line = [e_lines_by_node[i][network[i].electrical.connections.index(network[m].name)] for i in n]
#     return y_mn[m_line[RANGE],t] == -y_mn[n_line[RANGE],t]

# def line_electric_q(index):
#     t, m = index
#     m_line = e_lines_by_node[m]#index of line from m to n
#     n = [i for i in range(n_nodes) if network[i].name in network[m].electrical.connections]
#     n_line = [e_lines_by_node[i][network[i].electrical.connections.index(network[m].name)] for i in n]
#     return z_mn[m_line[RANGE],t] == -z_mn[n_line[RANGE],t]
...
#h_mn = VariableGroup("h_mn", indexes = index_h_lines)
#cooling network
#index_c_lines = range(n_c_lines), range(T)
#c_mn = VariableGroup("c_mn", indexes = index_c_lines)
...
# add_constraint("line_electric_p", (range(T), tuple(e_nodes)), line_electric_p)
# add_constraint("line_electric_q", (range(T), tuple(e_nodes)), line_electric_q)
