...
p_base = 100 # power base value MVA
current_base = p_base*1000/v_base 
r_base = v_base**2/(p_base*1000) # kohms nominal value to normalize admitance (kv^2/kVA = kohms)
j_base = r_base
v_nominal = 1.0#4135/4135
theta_nom = 10/180*(3.14159)
...
ep_dem = [0, 0.2, 0.2, 0.5, 0.6]
eq_dem = [0, 0.1, 0.15, 0.3, 0.4]
...
n_e_lines = i_e
n_lines_ordered = [2,6,0,7,9,12,1,3,10,4,8,13,5,11]
...
G = Y.real
B = Y.imag
Ymag = np.sqrt(np.square(B) + np.square(G))
theta = np.angle(Y)
G = np.multiply(Ymag,np.cos(theta))#*r_base
B = np.multiply(Ymag,np.sin(theta))#*j_base
...
def run_horizon(timestep, v_iters, x_n, pid_error_last):
    ...
    def electric_p_balance(index):
        t, m = index
        n = [i for i in range(n_nodes) if network[i].name in network[m].electrical.connections]#n = []# 
        i_turb = turbine_by_node[m]#i_turb = [0,1,2]# 
        i_lines = e_lines_by_node[m]
        i_slack = slack_by_node[m]
        ...
        if not n== []:
            ...
            - ep_dem[m]+cvxpy.sum([p_unserve[j,t] for j in i_slack])\
            == G[m,m]*x_m[m,t]\
    ...
    def electric_q_balance(index):
        ...
        if not n==[]:
            ...
            - eq_dem[m]+cvxpy.sum([q_unserve[j,t] for j in i_slack])\
            == - B[m,m]*x_m[m,t] + cvxpy.sum(G[m,n[RANGE]]*z_mn[i_lines[RANGE],t] - B[m,n[RANGE]]*y_mn[i_lines[RANGE],t])
        else:
            return cvxpy.sum([turbine_xq[j,t] for j in i_turb])\
            - 0.95\
            == 0
    ...
    def y_mn_equality(index):
        m_lines,t = index
        n_lines = n_lines_ordered[m_lines]
        # set those lines equal for y_mn
        return y_mn[m_lines,t] == y_mn[n_lines,t]
    ...
    def z_mn_inequality(index):
        ...
        return z_mn[m_lines,t] == -z_mn[n_lines,t]

    def y_voltage_limit_lower(index):
        m,t = index
        return (v_nominal*(1-voltage_deviation)*math.cos(theta_nom))**2 <= y_mn[m,t]

    def y_voltage_limit_upper(index):
        m,t = index 
        return y_mn[m,t] <= (v_nominal*(1+voltage_deviation))**2

    def z_voltage_limit_upper(index):
        m,t = index
        return z_mn[m,t] <= (np.sin(theta_nom)*v_nominal*(1+voltage_deviation))**2

    def z_voltage_limit_lower(index):
        m,t = index
        return -(np.sin(theta_nom)*v_nominal*(1+voltage_deviation))**2 <= z_mn[m,t]
    ...
    def known_angles_y(index):
        t, m = index
        return y_mn[m,t] == y_mn_known[m,t]

    def known_angles_z(index):
        t, m = index
        return z_mn[m,t] == z_mn_known[m,t]
    ...
    def turbine_y_consume(index):
        i, t = index
        return turbine_para[i].fundata["bp"]*cvxpy.power(turbine_xp[i,t],2)\
        + turbine_para[i].fundata["fp"]*turbine_xp[i,t]/p_base\
        - turbine_y[i,t] <= 0
    # + turbine_para[i].fundata["cp"]\


    def turbine_xp_lower(index):
        i, t = index
        #individual lower bounds are non-zero
        return (turbine_para[i].lb) <= turbine_xp[i,t]

    def turbine_xq_lower(index):
        i, t = index
        return 0.0 <= turbine_xq[i,t]

    def turbine_xp_upper(index):
        i, t = index
        return (turbine_para[i].ub) >= turbine_xp[i,t] 
        #+ cvxpy.power(turbine_xq_k[i,t],2)

    def turbine_xq_upper(index):
        i, t = index
        return (turbine_para[i].ub) >= turbine_xq[i,t]
    ...
    def p_unserve_upper(index):
        t, m = index
        return 2 >= p_unserve[m,t]
    ...
    if n_turbines>0:
        ...
        turbine_xq = VariableGroup("turbine_xq", indexes=index_turbines)  #  reactive power output
    ...
    q_unserve = VariableGroup("q_unserve", indexes = index_slack, lower_bound_func= constant_zero)
    p_unserve = VariableGroup("p_unserve", indexes = index_slack, lower_bound_func = constant_zero)
    # #heat network
    ...
    for i in range(1):
        var = q_unserve[i, 0]
        var1 = p_unserve[i,0]
        objective_components.append(var * 100)
        objective_components.append(var1 * 1000)
    ...
    add_constraint("y_mn_equality", index_e_lines, y_mn_equality)
    add_constraint("z_mn_inequality", index_e_lines, z_mn_inequality)
    ...
    add_constraint("turbine_xq_lower", index_turbine + index_hour, turbine_xq_lower)
    add_constraint("turbine_xq_upper", index_turbine + index_hour, turbine_xq_upper)
    add_constraint("turbine_xp_lower", index_turbine + index_hour, turbine_xp_lower)
    ...
    # add_constraint("y_voltage_limit_upper", index_e_lines, y_voltage_limit_upper)
    add_constraint("y_voltage_limit_lower", index_e_lines, y_voltage_limit_lower)
    # add_constraint("z_voltage_limit_upper", index_e_lines, z_voltage_limit_upper)
    # add_constraint("z_voltage_limit_lower", index_e_lines, z_voltage_limit_lower)
    ...
    # add_constraint("p_unserve_upper", index_hour + m_index, p_unserve_upper)
    ...
    tic = time.time()
    result = prob.solve(solver='MOSEK', verbose=True)#, feastol=1e-15)
    ...
    for m in range(n_e_nodes):
        if not (m==0):
            for t in range(T):
                ...
                x_n[m,t] = x_n[m,t] + voltage_error[m,t]*2/3#*(1-0.01/3)
